...
* Introduction
In the paper, we look into the problem fo real estate rental price estimation,
...
some characteristics that are valuable to consumers, but are currently neglected
...
variables in other unexpected ways, showing that the Neural Network captures at
least some previously unconsidered factors.

One of the main reason, why we think that the floorplans might contain
...
was collected to write this paper. The dataset we are using has over 140,000
observations, but only 3 categorical and 7 continuous variables including the
target variable. This is more observations, but less categories than in most
papers the author reviewed. We focused on rental apartments in Tokyo in
particular because, as outlined in textcite:moriizumi1986;weko_55303_1 and
textcite:akiyama_yuki2019320204, estimating real estate prices throughout
different markets are more challenging and causes complications as described
earlier. This paper is primarily an exploratory study, so we decided that by
focusing on "roughly" a single market, we can sidestep the problem of
considering multiple markets and focus on the viability of these computer vision
methods first. For each listing, we have the monthly rent of the apartment, the
image of the floorplan of the apartment, 6 continuous and 3 categorical
variables. The details for the tabular variables are described in Table
[[tab:var_explanation]].
...
showed a slight improvement in $R^{2}$, and the coefficients, the NN's
predictions improved significantly. We created dummy matrices for each of our
categorical variables, ending up with 724 columns, including the intercept and
continuous columns. The "station" variable's cardinality of 684 Moreover, the
"style" variable's cardinality of 31 caused this significant increase in
...
for the sake of brevity, we only show a single example in the summary.
...
The first floorplan in figure [[fig:upward]] is a floorplan for only a single room
depicted on the floor plan. The original listing was for a shared flat, and the
floorplan depicts all private rooms, as well as the public rooms everyone living
their may use. For this realestate property, the prediction of the model without
the NN's prediction is much more accurate than the other two. This is because
the Neural Network overestimated the rent by a lot. The reason for the NN's
overestimation might be that the neural network was considering the floorplan as
a whole, while the actual apartment available for rent is only a single room. A
similar problem seems to have occurred with the third floorplan, which displays
the apartment's floorplan and a balcony that everyone living in the building can
use. The reasons for the high upward shift in the second and fourth floorplans
are much harder to discern. The problem of the first and third images described
above, is correctly handled in other images outlined in the main paper.
...
with the new features. However, under any circumstances where it might be easier
to obtain floorplans of apartments rather than the tabular data of the
categorical features, an approach utilizing computer vision might be worth
considering. So our method could be used by entities who do not have the
