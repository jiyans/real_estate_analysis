...
Many studies in the actual literature use much bigger datasets, both column and
row-wise, and we believe that the inclusion of more variables used would reduce
the explanatory power of the model proposed in the current paper. For example,
different datasets might have a column for the number of bathrooms a given
property has. Should this turn out to be a significant variable, the explanatory
power of our Neural network in the final hedonic price model will probably
decrease. However, we believe that our model still has some practical advantages
because obtaining floorplans is often much more manageable than tabular data for
...
We will extend more on these points in the [[Background]] and [[Discussion]] sections.
*** Discuss outcomes
*** Stake a claim
** Evaluation
*** Analyze
